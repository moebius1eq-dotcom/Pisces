...
The homepage previews a 14.7-second camera flight through a deep universe of astronomy-filled jigsaw pieces: close encounters, a dramatic pullback revealing thousands of pieces, continuous assembly into unique connected slots, and one final missing piece. The assembled imagery resolves into **PISCES / PIECE TOGETHER THE UNIVERSE**; the final piece hinges open and the camera passes through into a minimal deep-space environment. Reload the homepage or use **Replay entrance** to review it locally. The preserved Journey remains reachable at `index.html#departure`; atlas URLs are unchanged. See [entrance notes](docs/pisces-entrance.md) for the storyboard, adaptive rendering, readiness, fallbacks and checks, and [image credits](assets/entrance-sources.json) for locally hosted NASA imagery. This milestone proves the passage and destination environment; the full homepage remains unbuilt.
